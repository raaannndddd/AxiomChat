...
//   'kill', 'rape', 'bomb', 'shoot', 'dox', 'address', 'ssn',
//   'swat', 'track', 'home address', 'phone number', 'murder'
// ];
// filter.addWords(...customBlocklist);

// // Core app/server setup
// const app = express();
// const httpServer = createServer(app);


// const accounts = {}; // Optional
// const users = {};
// const messageHistory = {};
// let anonCounter = 0;

// // Increase EventEmitter listener limit to avoid MaxListeners warning
...
// app.get('/', (req, res) => {
//   res.redirect('/livechat/livechat.html');
// });
...
//   try {
//     // Optional: fetch coin address from Axiom if available
//     const coinAddress = null; // TODO: integrate if Axiom exposes it

//     const data = await getCachedOrFetchCoinData(coinId, coinAddress);
//     res.json(data);
//   } catch (err) {
...
//     if (currentRoom) {
//       socket.to(currentRoom).emit('user-connected', anonName);
//     }
...
//       socket.emit('message-blocked', '⚠️ Message contains blocked content and was not sent.');
//       return;
//     }
...
//     if (currentRoom) {
//       messageHistory[currentRoom].push(payload);
...
//     if (currentRoom) {
//       socket.to(currentRoom).emit('user-disconnected', user);
//     }
...
//   try {
//     const result = await sentimentAnalyzer(message);
//     res.json(result[0]); // { label: 'POSITIVE', score: 0.999... }
//   } catch (error) {
//     res.status(500).json({ error: error.message });
//   }
// });

// app.post('/api/monitor/:coinId', (req, res) => {
//   if (!req.isAuthenticated()) return res.status(403).json({ error: "Not logged in" });

//   const userId = req.user.id; // or req.user._id if using Mongo later
//   if (!monitoredCoins[userId]) monitoredCoins[userId] = [];
//   if (!monitoredCoins[userId].includes(req.params.coinId)) {
//     monitoredCoins[userId].push(req.params.coinId);
//   }

//   res.json({ message: `Now monitoring ${req.params.coinId} for CTO.` });
// });
...
// app.listen(3001, () => {
//   console.log("Sentiment API on port 3001");
// });
